          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Input
                type="number"
                value={investimentos}
                onChange={e => setInvestimentos(Number(e.target.value))}
              />
            </div>
          </div>
          <table className="w-full text-left border-separate border-spacing-y-2 mt-6">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>- {formatCurrency(custos)}</td>
                <td>{((custos / receita) * 100).toFixed(1)}%</td>
              </tr>
              <tr>
                <td>Margem de contribuição</td>
                <td>+ {formatCurrency(receita - custos)}</td>
                <td>{(((receita - custos) / receita) * 100).toFixed(1)}%</td>
              </tr>
              <tr>
                <td>Despesas fixas</td>
                <td>- {formatCurrency(despesas)}</td>
                <td>{((despesas / receita) * 100).toFixed(1)}%</td>
              </tr>
              <tr>
                <td>LOAI</td>
                <td>+ {formatCurrency(loaiOriginal)}</td>
                <td>{((loaiOriginal / receita) * 100).toFixed(1)}%</td>
              </tr>
              <tr>
                <td>Investimentos</td>
                <td>+ {formatCurrency(investimentos)}</td>
                <td>{((investimentos / receita) * 100).toFixed(1)}%</td>
              </tr>
              <tr>
                <td>Lucro operacional</td>
                <td>+ {formatCurrency(lucroOriginal)}</td>
                <td>{((lucroOriginal / receita) * 100).toFixed(1)}%</td>
              </tr>
            </tbody>
          </table>
        </CardContent>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Input
                type="number"
                value={receitaVar}
                onChange={e => setReceitaVar(Number(e.target.value))}
              />
            ...
            <div>
              ...
              <Input
                type="number"
                value={custosVar}
                onChange={e => setCustosVar(Number(e.target.value))}
              />
              <p className="text-sm text-muted-foreground mt-1">
            ...
            <div>
              ...
              <Input
                type="number"
                value={despesasVar}
                onChange={e => setDespesasVar(Number(e.target.value))}
              />
              <p className="text-sm text-muted-foreground mt-1">
        ...
        <CardContent className="space-y-4 pt-6">
          ...
          <table className="w-full text-left border-separate border-spacing-y-2">
            ...
            <tbody>
              <tr>
                <td>Receita</td>
                <td>+ {formatCurrency(receitaNova)}</td>
                <td>100.0%</td>
                <td>{diferenca(receitaNova, receita)}%</td>
              </tr>
              <tr>
                <td>Custos variáveis</td>
                <td>- {formatCurrency(custosNovos)}</td>
                <td>{percentual(custosNovos)}%</td>
                <td>{diferenca(custosNovos, custos)}%</td>
              </tr>
              <tr>
                <td>Margem de contribuição</td>
                <td>+ {formatCurrency(margemFinal)}</td>
                <td>{percentual(margemFinal)}%</td>
                <td>{diferenca(margemFinal, receita - custos)}%</td>
              </tr>
              <tr>
                <td>Despesas fixas</td>
                <td>- {formatCurrency(despesasNovas)}</td>
                <td>{percentual(despesasNovas)}%</td>
                <td>{diferenca(despesasNovas, despesas)}%</td>
              </tr>
              <tr>
                <td>LOAI</td>
                <td>+ {formatCurrency(loaiNovo)}</td>
                <td>{percentual(loaiNovo)}%</td>
                <td>{diferenca(loaiNovo, loaiOriginal)}%</td>
              </tr>
              <tr>
                <td>Investimentos</td>
                <td>+ {formatCurrency(investimentos)}</td>
                <td>{percentual(investimentos)}%</td>
                <td>0.0%</td>
              </tr>
              <tr>
                <td><strong>Lucro operacional</strong></td>
                <td><strong>+ {formatCurrency(lucroNovo)}</strong></td>
                <td><strong>{percentual(lucroNovo)}%</strong></td>
                <td><strong>{diferencaLucro}%</strong></td>
              </tr>
            </tbody>
          </table>
