...
export default function SimuladorFinanceiro() {
  ...
  return (
    <div className="p-4 sm:p-6 grid grid-cols-1 gap-6 max-w-6xl mx-auto">
      {/* Card 1 */}
      <Card>
        <CardContent className="space-y-4 pt-6">
          <h2 className="text-xl font-bold">Resumo do Fluxo de Caixa do Período</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div><Label>Receita (R$)</Label><Input type="number" value={receita} onChange={e => setReceita(Number(e.target.value))} /></div>
            <div><Label>Custos Variáveis (R$)</Label><Input type="number" value={custos} onChange={e => setCustos(Number(e.target.value))} /></div>
            <div><Label>Despesas Fixas (R$)</Label><Input type="number" value={despesas} onChange={e => setDespesas(Number(e.target.value))} /></div>
            <div><Label>Investimentos (R$)</Label><Input type="number" value={investimentos} onChange={e => setInvestimentos(Number(e.target.value))} /></div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-separate border-spacing-y-2 mt-6 min-w-[600px]">
              <thead>
                <tr><th>Descrição</th><th>Média mensal</th><th>%</th></tr>
              </thead>
              <tbody>
      ...
      <Card>
        <CardContent className="space-y-4 pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div><Label>Receita (%)</Label><Input type="number" value={receitaVar} onChange={e => setReceitaVar(Number(e.target.value))} /><p className="text-sm text-muted-foreground mt-1">{formatCurrency(impactoReceita)}</p></div>
            <div><Label>Custos Variáveis (%)</Label><Input type="number" value={custosVar} onChange={e => setCustosVar(Number(e.target.value))} /><p className="text-sm text-muted-foreground mt-1">{formatCurrency(impactoCustos)}</p></div>
            <div><Label>Despesas Fixas (%)</Label><Input type="number" value={despesasVar} onChange={e => setDespesasVar(Number(e.target.value))} /><p className="text-sm text-muted-foreground mt-1">{formatCurrency(impactoDespesas)}</p></div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4 pt-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-separate border-spacing-y-2 min-w-[700px]">
              <thead>
                <tr><th>Descrição</th><th>Média mensal</th><th>%</th><th>Diferença</th></tr>
              </thead>
              <tbody>
